...
function unsafe_transpile(ast::Expr, run_benchmarks::Bool=false)::Ptr{Cvoid}
    GC.@preserve ast begin
        handle = ccall((:stc_jl_transpile, libstc), Ptr{Cvoid}, (Any, Bool), ast, run_benchmarks)
    end
...
function unsafe_get_transpiler_result(handle::Ptr{Cvoid})::Ptr{Cchar}
    ...
    return ccall((:stc_jl_cstr_from_handle, libstc), Ptr{Cchar}, (Ptr{Cvoid},), handle)
end
...
function unsafe_free_transpiler_handle(handle::Ptr{Cvoid})
    handle != C_NULL && ccall((:stc_jl_free_handle, libstc), Cvoid, (Ptr{Cvoid},), handle)
end
